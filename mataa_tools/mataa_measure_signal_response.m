...
function [dut_out,dut_in,t,dut_out_unit,dut_in_unit,X0_RMS] = mataa_measure_signal_response (X0,fs,latency,verbose,channels,cal,X0_unit);
  ...
  if exist('mataa_playrecord_pa19')
    uTestToneImplementation = 1;
  else
    uTestToneImplementation = 0;
  end
  ...
  switch uTestToneImplementation
    case 0
    ...
    case 1
      ...
      numZeroSamples = latency * fs;
      zeroSamples    = repmat(padding_level,round(numZeroSamples),1);
      zeroSamples    = repmat(zeroSamples,1,columns(X0));
      dut_in         = [zeroSamples; X0; zeroSamples];
      % Defaultwerte für alles, aber 100ms latency (wie audacity)
      out     = mataa_playrecord_pa19(dut_in, fs, 100e-3);
      t       = out(:,1);
      dut_out = out(:,2:end);
  end
